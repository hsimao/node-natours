...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiaHNpbWFvIiwiYSI6ImNrMzYwODJtNzBueHQzY3Blb2hodXZsMDkifQ.PEJGnj7qoBvoV68p7wQSxw';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/hsimao/ck360c7fw13wa1cmbflkff42r',
    scrollZoom: false // center: [-118.35607, 34.140441],
    // zoom: 9
    // interactive: false // 禁止用戶改變 map

  });
  var bounds = new mapboxgl.LngLatBounds(); // loop 所有座標

  locations.forEach(function (location) {
    // 建立地圖座標 dev
    var el = document.createElement('div');
    el.className = 'marker'; // 添加地圖座標 dev 到 mapbox marker
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(location.coordinates).setHTML("<p>Day ".concat(location.day, ": ").concat(location.description, "</p>")).addTo(map); // 擴充地圖可視範圍範圍, 需包含當前位置
...
var showAlert = function showAlert(type, msg) {
  hideAlert();
  var markup = "<div class='alert alert--".concat(type, "'>").concat(msg, "</div>"); // 在 body 標籤下方最開始插入 markup
...
/* eslint-disable */
var stripe = Stripe('pk_test_rfkuG3cOIO0gxi00olKeEify00x2DJ61wW');
    ...
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          _context.prev = 0;
          _context.next = 3;
          return regeneratorRuntime.awrap((0, _axios.default)("/api/v1/booking/checkout-session/".concat(tourId)));
